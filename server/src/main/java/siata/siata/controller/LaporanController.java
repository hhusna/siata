...
public class LaporanController {
    ...
    @GetMapping(value = "/laptop-needs/pdf", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<InputStreamResource> exportLaptopNeedsPdf() {
        ByteArrayInputStream bis = laporanService.createLaptopNeedsLetter();

        HttpHeaders headers = new HttpHeaders();
        String tgl = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
        headers.add("Content-Disposition", "inline; filename=SuratKebutuhanLaptop_" + tgl + ".pdf");
